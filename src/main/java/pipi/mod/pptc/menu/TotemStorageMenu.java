...
public class TotemStorageMenu extends WithInventoryMenu {
	public static final Identifier EMPTY_SLOT_TOTEM = PPTC.locate("slot/totem");
	...
	public TotemStorageMenu(int containerId, InteractionHand hand, ITotemStorage totemStorage, Inventory playerInv) {
		...
		this.selected = playerInv.getSelectedSlot();
		this.holdHand = hand;
	...
	public void clicked(int slotId, int button, ClickType clickType, Player player) {
		...
		if(this.isLockedSlot(slotId, true)
				// 数字キーやfキーでの移動に注意
				|| (clickType == ClickType.SWAP && this.isLockedSlot(button, false))) {
			return;
		}
